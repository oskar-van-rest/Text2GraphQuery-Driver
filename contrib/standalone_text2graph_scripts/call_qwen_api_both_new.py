...
# --- ISO GQL (Standard) ---
    gql_system = (
        ...
        "Example 1\n"
        "Question: Which characters have a path to \"Catelyn-Stark\" in the interaction network with a maximum of 3 hops?\n"
        "Output: MATCH (target:Person)-[:CAST_FOR]->(m:Movie)<-[:CAST_FOR]-(p:Person) FILTER target.name = 'Tom Hanks' AND p.vid <> target.vid RETURN DISTINCT p.name\n\n"
        "Example 2\n"
        "Question: How many people have directed more than two movies?\n"
        "Output: MATCH (p:Person)-[r:CREW_FOR]->(m:Movie) FILTER r.job = 'Director' RETURN p.vid AS personVid, COUNT(m) AS moviesDirected NEXT FILTER moviesDirected > 2 RETURN COUNT(*) AS directorsCount\n\n"
        "Example 3\n"
        "Question: List the top 5 movies with the most production companies involved.\n"
